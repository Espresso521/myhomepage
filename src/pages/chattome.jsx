...
const socketio = (setStatus, addMsg) => {
  ...
  ws.onmessage = function (evt) {
    // {"protocal":"msg","sender":"Pure Spread","receiver":"","content":"aaaaa","icon":"/8avatar.svg","append":""}
    console.log("RESPONSE: " + evt.data)

    const msg = JSON.parse(evt.data)
    if (msg.sender === myName && msg.icon === myIcon) return
...
function Chat () {
  ...
  const getTag = (status) => {
    if (status.status === 1) {
      return (<Tag color="green"><LinkOutlined /></Tag>)
    } else {
      return (<Tag color="red"><DisconnectOutlined /></Tag>)
    }
  }
